Version 2026-09-13 14:43 (Europe/Paris)
...
- ⏳ **Vue objectif** — la taille est désormais harmonisée avec les deux autres vues (V020), pas encore ancrée, à faire plus tard. Important : le schéma de l'objectif est inséré dans un espace carré alors qu'il est rectangulaire, il est donc déformé (compression horizontale), il est urgent de réparer cette situation.
...
1. **Vue Objectif**, corriger la compression horizontale du schéma de l'objectif, puis correction de l'outil `editeur-ancres-vue-objectif.html` (intégration du schéma rectangulaire si nécessaire et vérification de l'adéquation des variables utilisees avec les contrôles réels).
