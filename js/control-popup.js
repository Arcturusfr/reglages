...
function openControlDetail(ctrlId,ctx){
  ...
  const ctxHtml=ctx&&ctx.param&&ctx.value
    ?`<div class="ctrl-context"><span class="ctrl-context-label">Réglage concerné</span><span class="ctrl-context-val">${esc(ctx.param)} · ${esc(ctx.value)}</span></div>`:'';
  ...
  document.getElementById('ctrl-popup').innerHTML=`
    ...
    <div class="popup-body">
      ${ctxHtml}
      <figure class="ctrl-figure">${d.svg}${d.caption?`<figcaption class="ctrl-figure-caption">${esc(d.caption)}</figcaption>`:''}</figure>
      ${sectionsHtml}
      ${tipsHtml}
